...
Build **[llama.cpp](https://github.com/ggml-org/llama.cpp)** on **Ubuntu 24.04 (amd64)** with **Vulkan**, **ROCm (HIP)**, and **CUDA**, install each backend under a shared prefix, bundle needed shared libraries (rpath + copies), and emit one **`llama.cpp-*-v*-linux-x86_64.tar.xz`** in the repo root. That archive is a relocatable tree, not a single static binary.
